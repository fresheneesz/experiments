...
{	if(seed == -1)
	...
	//SSL_library_init();		// NOT ENTIRELY SURE THIS IS NECCESSARY
}

// returns 16 byte hash
inline char* MD5hash(char* in, int size, char* out)
{	MD5((unsigned char*)in, size, (unsigned char*)out);
	out[16]=0; // remember 0's might be in the middle of the hash too
	return out;
}
...
{public:
	...
	{	BIGNUM* e = BN_new();
		...
		*/
		RSA_generate_key_ex(key, bytes*8, e, 0);
	}
	...
	int size(RSAkey* key)
	{	return RSA_size(key->key);
	}
	
	// loads PEM formated private (and public) keys
	bool loadPEMpriv(void* data, int len)
	{	BIO *keymem;
        keymem = BIO_new_mem_buf(data, len);

		RSA* tempkey;
		tempkey = PEM_read_bio_RSAPrivateKey(keymem, 0, 0, 0);
	...
	// loads a PEM formated public key
	bool loadPEMpub(void* data, int len)
	{	BIO *keymem;
        keymem = BIO_new_mem_buf(data, len);

		RSA* tempkey;
		tempkey = PEM_read_bio_RSAPublicKey(keymem, 0, 0, 0);
	...
	// alternate format used for the format given
	// by the "openssl rsa" command: openssl rsa -in rsa.private -out rsa.public -pubout -outform PEM
	bool loadPEMpubAlt(void* data, int len)
	{	BIO *keymem;
        keymem = BIO_new_mem_buf(data, len);
		if(keymem == 0)
			return true;
		...
		RSA* tempkey;
		tempkey = PEM_read_bio_RSA_PUBKEY(keymem, 0, 0, 0);
	...
	// saves to a file - eventually would want to save to arbitrary place in memory
	...
	{	// the buffer 'back' must be at least the size of the return value of RSA_size(keys)
		...
		return RSA_public_encrypt(mlen, (unsigned char*)message, (unsigned char*) encryptedMessage, key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	...
	{	// the buffer 'back' must be at least the size of the return value of RSA_size(keys)
		...
		// less than RSA_size(rsa) - 41 for RSA_PKCS1_OAEP_PADDING 
		// and exactly RSA_size(rsa) for RSA_NO_PADDING.
		return RSA_private_encrypt(mlen, (unsigned char*)message, (unsigned char*) encryptedMessage, key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// return -1 on fail - uses RSA_PKCS1_OAEP_PADDING which might be slower than others
	bool decPub(char* encryptedMessage, int encryptedmlen, char* originalMessage)
	{	return RSA_public_decrypt(encryptedmlen, (unsigned char*)encryptedMessage, (unsigned char*)originalMessage, key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
	
	// return -1 on fail - uses RSA_PKCS1_OAEP_PADDING which might be slower than others
	bool decPriv(char* encryptedMessage, int encryptedmlen, char* originalMessage)
	{	return RSA_private_decrypt(encryptedmlen, (unsigned char*)encryptedMessage, (unsigned char*)originalMessage, key, RSA_PKCS1_PADDING/*RSA_PKCS1_OAEP_PADDING*/);
	}
