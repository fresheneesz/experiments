...
Rescripted("Rescripted.Lang.ToRescripted",["Rescripted.Lang.CodePrinter"],function(CodePrinter){
	...
	//  		current.println(current.tree.children[0],"Attempt to assign to a val")
		...
		PartialFunction:function(cases,current){
			...
			cases.forEach(function(caseStatement){
				...
				function processPattern(pattern){
					switch(pattern.type){
						case "extractor":
							code.write((pattern.name == null ? "" : (pattern.name+"@"))+pattern.extractor+"(")
							...
							code.write(")");
							break;
						case "typed": code.write(pattern.name+":"+pattern.requiredType.toString()); break;
						case "equals":
		...
		String:function(value,rawValue){
			code.write(rawValue)
		},
	...
	function PrintCode(){ console.log(code.getCode()); }
	function GetCode(){return code.getCode() };
