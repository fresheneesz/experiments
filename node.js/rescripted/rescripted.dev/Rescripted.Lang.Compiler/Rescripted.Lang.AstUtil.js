...
Rescripted("Rescripted.Lang.AstUtil",["Rescripted.Lang.Antlr","Rescripted.Lang.Lexer","Rescripted.Lang.Parser","Rescripted.Lang.Json"],function(org,RescriptedLexer,RescriptedParser,Json){
	...
	function RescriptedParse(source){
	...
	function DecodeStringToken(value){
		var isTripleQuote = (/^("""|''')/.test(value));
		if(isTripleQuote)
			return DecodeTripleQuotedString(value)
		else
			return DecodeSingleQuotedString(value)
	}
	
	function DecodeTripleQuotedString(value){
		value = value.substring(3,value.length - 3);
		var frontMargin = /^\n(\s+)/.exec(value)
		var endMargin = /\n([ \t]+)$/.exec(value)
		if(frontMargin == null || endMargin == null) //one of the margins didn't match, not valid.
			return value;
		if(frontMargin[1].length < endMargin[1].length) //front margin must be as long as end margin
			return value
		
		var lines = value.split(/\n/).slice(1,-1); //ignore the first and last lines
		var minMargin = endMargin[1].length;
		lines.forEach(function(line){ minMargin = Math.min(/^(\s*)/.exec(line)[1].length, minMargin) })
		if(minMargin < endMargin[1].length) // all other lines must be at least as long as end margin
			return value;
		
		//should probably check contents of margin, not just length, otherwise tabs will mess things up
		
		return lines.map(function(line){return line.substring(minMargin,line.length)}).join("\n");
	}
	
	function DecodeSingleQuotedString(value){
		return eval(value); //horible dirty hack, depends on our string format being the same as javascript...
	}
		...
		function Walk(tree,context){
			...
			function Visit(type,newContext,node){
				...
				args.push({
					...
					recurse: function(index, overrideContext){ return index == null ? Walk(nodes,overrideContext || newContext || context) : Walk(nodes[index],overrideContext || newContext || context) },
					walk:function(n,overrideContext){return Walk(n,overrideContext || newContext || context)}
				})
			...
			function ObjectPairs(tree){
				...
				return (tree.children || []).map(function(pair){
					var key = pair.children[0];
					var decodedKey = (key.token.type == tokens.STRING)? DecodeStringToken(key.getText()): key.getText();
					var encodedKey = Json.encodeString(decodedKey);
					if(keys.hasOwnProperty(decodedKey))
						println("Duplicate key in object literal: "+encodedKey)
					keys[decodedKey] = true;
					return {key:encodedKey,value:pair.children[1]}	
				});
			}
			...
			switch(tree.token.type){
						...
						function preparePattern(pattern){
							switch(pattern.token.type){
								case tokens.EXTRACTOR_PATTERN:
									return {
										type: "extractor",
										name: IncludeTokens(pattern.children,"ID").map(function(id){return id.getText()})[0],
								...
								case tokens.TYPED_PATTERN: return {type:"typed",name:IncludeTokens(pattern.children,"ID")[0].getText(),requiredType:Type(IncludeTokens(pattern.children,"TYPE")[0])};
				...
				case tokens.STRING: return Visit("String",null,tree,DecodeStringToken(tree.getText()),tree.getText());
		...
		function Walk(tree,context,metadata,parent,imports,scope){
			...
			switch(tree.token.type){
				...
				case tokens.DEF:
					...
					var nameNode = IncludeTokens(tree.children,"METHOD_NAME")[0].children[0];
					var name = nameNode.getText()
					var isOperator = nameNode.token.type == tokens.OPERATOR;
					...
					return Visit("Def",context.create({in:"method"}),children,Metadata(),name,isOperator,argDeclarations,type);
	...
	return {
		println: println,
		RescriptedParse: RescriptedParse,
		PrintAst: PrintAst, 
		TokenName: TokenName,
		FindTokens: IncludeTokens,
