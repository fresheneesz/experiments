...
function generateShip(x,y,type,size) {
    var ship = new Ship(x,y,type,size);

    if (type == "fighter") {
	var line = addLine(size,120,150,5*size,ship);
	line.weaponAt = "begin";
	line.weapon = new Weapon("laser",1,4,5,"#ff0000");
	line = addLine(size,240,30,5*size,ship);
	line.weaponAt = "end";
	line.weapon = new Weapon("laser",1,4,5,"#ff0000");
	addLine(size,0,270,5*size,ship);
	ship.radius = 7;
    } else if (type == "station") {
	addShip(size,90,0,15*size,ship,"fighter");
	addShip(size,30,60,15*size,ship,"fighter");
	addShip(size,330,120,15*size,ship,"fighter");
	addShip(size,270,180,15*size,ship,"fighter");
	addShip(size,210,240,15*size,ship,"fighter");
	addShip(size,150,300,15*size,ship,"fighter");
	ship.radius = 22;
    }
...
function addLine(size,rot,posrot,len,ship) {
    ...
    var line = new Line(size,rot,posrot,len,lx,ly);
...
function addShip(size,rot,posrot,len,ship,type) {
    ...
    var newShip = generateShip(lx,ly,type,size);
    newShip.rotate(rot);
